            ...
            <div className={styles.controlsContainer}>
                <div className={styles.filterContainer}>
                    <input type="text" placeholder="Filtrar por PID, Nome, Usuário..." value={searchTerm}
                        onChange={(e) => { setSearchTerm(e.target.value); if (!showAll) setVisibleCount(INITIAL_ITEMS_TO_SHOW); }}
                        className={styles.filterInput} />
                </div>
                <div className={styles.viewOptionsContainer}>
                    <button onClick={toggleShowAll} className={styles.actionButton}> {showAll ? "Recolher Lista" : `Mostrar Todos (${filteredProcessCount})`} </button>
                </div>
            </div>
                ...
                <table>
                    <thead>
                        <tr>
                            ...
                            <th onClick={() => requestSort('ppid')}>PPID{getSortIndicator('ppid')}</th>
                            <th onClick={() => requestSort('priority')}>Prioridade{getSortIndicator('priority')}</th>
                            <th onClick={() => requestSort('nice')}>Nice{getSortIndicator('nice')}</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        {displayedProcesses.length === 0 && !loading && (<tr><td colSpan="10" className={styles.noDataCell}>Nenhum processo encontrado.</td></tr>)}
                    </tbody>
                </table>
            </div>
